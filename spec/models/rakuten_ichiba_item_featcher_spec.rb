describe 'RakutenIchiba::ItemFetcher' do
  ...
  describe 'self.fetch' do
    ...
    context '正常にfetchできたとき' do
      ...
      it '内容が期待通りであること' do
        ...
        wait_max 1.0 do
          @items.count.should.equal @response[:Items].count
          @items.each_with_index do |item, index|
            item.name.should.equal @response[:Items][index][:Item][:ItemName]
            item.image_url.should.equal @response[:Items][index][:Item][:smallImageUrls]
          end
        end
      end
    end
